...
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Contrato-Autorizacao.aspx.cs" Inherits="admin.bepiducb.com.br.Private.Contrato_Autorizacao" %>
...
AUTORIZA o Sr. <%= Session["N"].ToString() %>, portador da cédula de identidade RG nº <%= Session["Identidade"].ToString() %>, <%= Session["Orgao"].ToString() %> e do CPF/MF nº <%= Session["C"].ToString() %>, 
residente e domiciliado na <%= Session["Endereco"].ToString() %>, <%= Session["Cidade"].ToString() %>/<%= Session["Estado"].ToString() %>, CEP <%=Session["CEP"].ToString() %>, 
a utilizar os equipamentos <b>(“Equipamentos”)</b> abaixo relacionados fora das dependências do Programa Brasileiro para desenvolvimento em iOS (“BEPiD”), 
...
A autorização ora concedida está subordinada ao <b>INSTRUMENTO PARTICULAR DE CESSÃO DE EQUIPAMENTOS</b> celebrado entre 
o <b>Sr. <%= Session["N"].ToString() %></b> e o <b>ELDORADO</b>, em 24 de Março de 2014.
Desta maneira, reitera-se que:

    (i) O <b>Sr. <%= Session["N"].ToString() %></b> compromete-se a utilizar os <b>Equipamentos</b>, exclusivamente, para os fins a que se destina o Programa 
        Brasileiro para desenvolvimento em iOS (“<b>BEPiD</b>”), nos limites da autorização ora concedida, respeitando as características e instruções 
        constantes dos respectivos manuais técnicos, bem como aquelas constantes em suas embalagens.

    (ii) O <b>Sr. <%= Session["N"].ToString() %></b> deverá manter e guardar os <b>Equipamentos</b> recebidos, assim como as respectivas embalagens e manuais, 
         como se seus fossem, desde o recebimento até a efetiva devolução ao <b>ELDORADO</b>, ficando responsável pela conservação e obrigando-se a 
         devolvê-los tão logo seja notificado para tanto.

    (iii) O <b>Sr. <%= Session["N"].ToString() %></b> compromete-se a restituir os <b>Equipamentos</b> ao <b>ELDORADO</b>, em perfeito estado, limpos e nas mesmas 
    ...
    (iv) Considerando que os <b>Equipamentos</b> se encontram devidamente segurados, em âmbito nacional, contra furto qualificado e contra danos acidentais, 
         exceto para a hipótese de derramamento de líquidos de qualquer natureza, o <b>Sr. <%= Session["N"].ToString() %></b> obriga-se: 
    ...
    (v) O <b>Sr. <%= Session["N"].ToString() %></b> declara estar ciente de que os Equipamentos são rastreáveis eletronicamente, bem como se obriga a não 
        promover qualquer alteração de software ou hardware que possa comprometer ou inabilitar o sistema de rastreamento instalado. 
